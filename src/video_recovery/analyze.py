...
from video_recovery.mp4_boxes import (
    find_all,
    format_size,
    parse_tfdt,
    parse_tfhd,
...
def analyze_container(path: Path) -> dict:
    ...
    moov_after_mdat = (
        moov_offset is not None and first_mdat is not None and moov_offset > first_mdat
    )
    ...
    if container.get("is_fragmented"):
        report.add(
            Finding(
                ...
                "Remux to a progressive MP4 with interleaved A/V.",
            )
        )
...
def analyze(path: Path, *, decode_scan: bool = False) -> AnalysisReport:
    ...
    if any(x in fmt_name for x in ("mp4", "mov", "isom", "m4a", "3gp")):
        ...
        report.container = {
            "note": f"Non-MP4 container ({fmt_name}); box analysis skipped"
        }
    ...
    if c.get("is_fragmented") is not None:
        ...
        if c.get("top_level"):
            ...
            for b in c["top_level"]:
                print(
                    f"  0x{b['offset']:08X}  {b['type']:4s}  {format_size(b['size'])}"
                )
